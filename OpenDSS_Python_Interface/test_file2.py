# -*- coding: utf-8 -*-
"""
Created on Mon Aug 08 10:42:37 2016
...
def DSS_Python_Interface1(filepath, contingency_range):
    ...
    line_to_be_removed = maptest9bus_test_system1(contingency_range);
...
#        Line_Currents.append(np.array(curr_Line_Current));
    ...
    for i in range(0, line_to_be_removed_size):
        ...
        for j in range(0, size_of_each_contingency):
            stagecounter = 0;
            DSSCircuit.CktElements(line_to_be_removed[i][j]).Open(1,0);
            DSSCircuit.CktElements(line_to_be_removed[i][j]).Open(2,0);
            Total_line_loss_counter = size_of_each_contingency;
            initial_loss.append(line_to_be_removed[i][j]);
